...
class Phase1Model:
    ...
    def test(self, verbose=True, num_to_display=20, error_margin=10):
        print("Testing model...")
        results = self.model.evaluate(self.dataset.testing_features, self.dataset.testing_labels, batch_size=self.batch_size, verbose=0)

        print("Generating predictions...")
        predictions = self.model.predict(self.dataset.testing_features, verbose=0)
        unscaled_predictions = []
        unscaled_testing_labels = []
        count = 0
        for p in range(len(predictions)):
            norm_vals = self.dataset.file_norm_vals[self.dataset.testing_files[p]]
            unscaled_prediction = normalize.unscale_from_range(predictions[p], norm_vals[0], norm_vals[1], -1, 1)
        ...
        if verbose:
            for i in range(len(self.model.metrics_names)):
                print(self.model.metrics_names[i] + ": " + str(results[i]))
            display_data = []
            ...
            print("% predictions with error > " + str(error_margin) + ": " + str(count / len(predictions)))
